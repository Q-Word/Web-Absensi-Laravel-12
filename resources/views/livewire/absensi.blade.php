<div>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
      {{-- Overlay --}}
      <div class="absolute inset-0 bg-gray-500 opacity-75 z-40 pointer-events-none"></div>
      {{-- Modal Box --}}
      <div class="relative z-50 inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 border border-red-400">
    ...
    <div class=" p-6 bg-[#F5F5F5] border border-gray-200 rounded-lg dark:bg-[#1B1B1B] dark:border-gray-700 shadow-[0px_10px_1px_rgba(221,_221,_221,_1),_0_10px_20px_rgba(204,_204,_204,_1)] dark:shadow-none">
      ...
      <div class="flex md:flex-row flex-col w-[100%]">
        <div class="bg-teal-500 w-full">
          <div id="{{ !$isOnLeave? 'map': '' }}" class= "h-96" wire:ignore>
              ...
              <div class="flex">
                ...
                <div class="ms-3">
                  ...
                  <p class="text-sm text-gray-700 dark:text-neutral-100">
                    {{ $wfa? 'Anda punya hak WFA(Work From Anywhere)': 'Anda Berada di Lingkungan Kantor' }}. Silahkan Melanjutkan Proses absensi.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            <div class="flex gap-2 flex-col sm:flex-row">
              <form wire:submit='store' enctype="multipart/form-data" class="flex gap-2 w-full">
                <flux:button type="button" icon="map-pin" id="{{!$isOnLeave && !$todayIsWeekend ? 'btnTagLocation': '' }}" class="w-full bg-amber-500! hover:bg-amber-400!">Submit Lokasi</flux:button>
                @if ($insideRadius)
            ...
            <div class="text-xs text-red-600">
              @if($todayIsWeekend)
                *Hari Ini sedang weekend, nikmati weekend anda.
              @elseif($isOnLeave)
                *Anda sedang dalam masa cuti, anda dapat kembali ke dashboard atau tetap di halaman absensi (namun tidak dapat submit lokasi).
              @else
                *validasi lokasi sebelum bisa absensi
              @endif
            </div>
          </div>
        </div>
